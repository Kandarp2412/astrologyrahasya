...
const VariantTableRow = (props) => {
  ...
  const handleSaveQuantity = () => {
    onSaveQuantity?.(variant.id, Number.parseInt(quantity, 10), mode);
    setQuantity('');
  };
    ...
    <TableRow {...other}>
      ...
      <TableCell sx={{ width: 333 }}>
        <Box sx={{ display: 'flex' }}>
          ...
          <Button
            color="primary"
            disabled={quantity.length === 0 || (quantity < 1 && mode === 'add') || (quantity
              < 0
              && mode
              === 'set')}
...
export const ProductInventory = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Product Inventory | Carpatin Retail Dashboard</title>
      </Helmet>
      <Box
        sx={{
